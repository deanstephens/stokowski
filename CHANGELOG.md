...
- feat: **create tickets from Slack** (`slack.ticket_creation: true`) — @-mention the bot with a rough idea and it drafts the ticket conversationally in a thread (clarifying questions + acceptance criteria), then files it into the project's Todo state on **Create ticket** so the daemon picks it up automatically. New module `stokowski/drafting.py`; `runner.run_oneshot` for stateless drafting turns; `LinearClient.create_issue`. Uses the existing message events (no new Slack scope); `slack.ticket_project` selects the target for multi-project setups.
- feat: **Linear card links to the Slack thread** — when a gate posts to Slack, Stokowski resolves the message permalink (`chat.getPermalink`) and adds a `💬 Slack thread: …` comment to the Linear issue, so the link is bidirectional (the gate message already has a "View issue" button back to Linear). Posted once per thread; best-effort.
...
- feat: agent Claude Code session ids are now persisted to `.stokowski/session` in each workspace — sessions survive orchestrator restarts and are resumed on the next turn (`claude --resume`).
